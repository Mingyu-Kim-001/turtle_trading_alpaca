"""
Turtle Trading Backtester with Long and Short Positions (55-day Entry, 20-day Exit)

This backtester implements the Turtle Trading strategy with support for:
- Long positions: Entry on 55-day high, exit on 20-day low, stop at entry - 2N
- Short positions: Entry on 55-day low, exit on 20-day high, stop at entry + 2N
...
class TurtleLongShort55_20Backtester:
  ...
  def _calculate_indicators(self, df):
    ...
    # Calculate 55-day entry and 20-day exit Donchian channels
    df = IndicatorCalculator.calculate_donchian_channels(df, entry_period=55, exit_period=20)
    return df
  ...
  def _process_exits(self, processed_data, today_date, yesterday_date):
    ...
    # Exit long positions (20-day low exit)
    for ticker in list(self.long_positions.keys()):
      ...
      # Exit signal (price falls below 20-day low)
      elif today['close'] < yesterday['low_20']:
    ...
    # Exit short positions (20-day high exit)
    for ticker in list(self.short_positions.keys()):
      ...
      if today['high'] >= position['stop_price']:
        ...
        exit_signal = True
        exit_reason = 'Stop-loss'
      # Exit signal (price rises above 20-day high)
      elif today['close'] > yesterday['high_20']:
        exit_price = today['close']
        exit_signal = True
  ...
  def _process_entries(self, processed_data, today_date, yesterday_date, total_equity):
    ...
    for ticker, df in processed_data.items():
      ...
      # Check for long entry signal (breakout above 55-day high)
      if today['high'] > yesterday['high_55']:
        if not self.last_trade_was_win.get((ticker, 'long'), False):
          ...
          # Reset win flag if price breaks below 55-day low
          if today['low'] < yesterday['low_55']:
            self.last_trade_was_win[(ticker, 'long')] = False

      # Check for short entry signal (breakdown below 55-day low)
      if self._is_shortable(ticker) and today['low'] < yesterday['low_55']:
        if not self.last_trade_was_win.get((ticker, 'short'), False):
          ...
          # Reset win flag if price breaks above 55-day high
          if today['high'] > yesterday['high_55']:
...
if __name__ == "__main__":
  ...
  backtester = TurtleLongShort55_20Backtester(
  ...
  print(f"Strategy: Turtle Trading 55-20 {'Long + Short' if ENABLE_SHORTS else 'Long Only'}")
  print(f"Entry Signal: 55-day high/low breakout")
  print(f"Exit Signal: 20-day low/high")
  ...
  plot_dir = os.path.join(project_root, 'backtesting', 'turtle_long_short_55_20_plots')
  ...
  if cash_history:
    ...
    plt.title('Cash Over Time (55-20 Strategy)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Cash ($)')
  ...
  if equity_history:
    ...
    plt.axhline(y=total_initial_equity, color='r', linestyle='--', label='Initial Equity')
    plt.title('Total Equity Over Time (55-20 Long + Short)', fontsize=14)
    plt.xlabel('Date')
  ...
  if net_unit_history:
    ...
    plt.title('Position Units Over Time (55-20 Strategy)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Units')
    plt.legend()
    plt.grid(True, alpha=0.3)
