...
def run_single_backtest(args):
    ...
    try:
        ...
        final_equity, all_trades, final_cash, _, _, _, _, _ = results
...
Examples:
  ...
  python compare_systems_parallel.py --risk-per-unit 0.01
    ...
    system1_pnls, system1_returns = print_stats(system1_results, "SYSTEM 1 (20-10)")
    dual_pnls, dual_returns = print_stats(dual_results, "DUAL SYSTEM (20-10 + 55-20)")
    ...
    t_stat, p_value = stats.ttest_rel(dual_pnls, system1_pnls)
    ...
    if pnl_diff > 0 and p_value < 0.05:
    ...
    elif pnl_diff < 0 and p_value < 0.05:
        ...
        print(f"  - Difference is not statistically significant (p={p_value:.6f})")
        print(f"  - Consider other factors (simplicity, trade frequency, etc.)")
