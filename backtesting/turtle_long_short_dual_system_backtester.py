"""
Turtle Trading Backtester with Dual System (20-10 + 55-20) Long and Short Positions

This backtester implements BOTH Turtle Trading systems simultaneously:
...
class TurtleDualSystemBacktester:
  ...
  def _calculate_indicators(self, df):
    ...
    # This will calculate high_20, low_20, high_55, low_55, high_10, low_10
  ...
  def _open_long_position(self, ticker, entry_price, n_value, total_equity, today_date, system):
    ...
    self.long_positions[ticker] = {
      ...
      'pyramid_count': 1,
      'side': 'long',
      'system': system  # Track which system opened this position
    }
    self.cash -= cost
  ...
  def _close_long_position(self, ticker, exit_price, today_date, exit_reason):
    ...
    if self.enable_logging:
      ...
      print(f"\n{'='*80}")
      print(f"EXIT LONG: {ticker} [{exit_reason}] System {position['system']}")
      print(f"{'='*80}")
  ...
  def _close_short_position(self, ticker, exit_price, today_date, exit_reason):
    ...
    if self.enable_logging:
      ...
      print(f"\n{'='*80}")
      print(f"EXIT SHORT: {ticker} [{exit_reason}] System {position['system']}")
      print(f"{'='*80}")
      print(f"  Entry Date:       {position['entry_date'].strftime('%Y-%m-%d')}")
      print(f"  Exit Date:        {today_date.strftime('%Y-%m-%d')}")
      print(f"  Duration:         {duration} days")
...
if __name__ == "__main__":
  ...
  if all_trades:
    ...
    system1_trades = [t for t in all_trades if t['system'] == 1]
    system2_trades = [t for t in all_trades if t['system'] == 2]
    ...
    # System breakdown
    if system1_trades:
      s1_wins = [t for t in system1_trades if t['pnl'] > 0]
      s1_pnl = sum(t['pnl'] for t in system1_trades)
      s1_win_rate = (len(s1_wins) / len(system1_trades)) * 100
      print(f"\nSystem 1 (20-10) Summary:")
      print(f"  Total: {len(system1_trades)}")
      print(f"  Win Rate: {s1_win_rate:.2f}%")
      print(f"  Total P&L: ${s1_pnl:,.2f}")

    if system2_trades:
      s2_wins = [t for t in system2_trades if t['pnl'] > 0]
      s2_pnl = sum(t['pnl'] for t in system2_trades)
      s2_win_rate = (len(s2_wins) / len(system2_trades)) * 100
      print(f"\nSystem 2 (55-20) Summary:")
      print(f"  Total: {len(system2_trades)}")
      print(f"  Win Rate: {s2_win_rate:.2f}%")
      print(f"  Total P&L: ${s2_pnl:,.2f}")
  ...
  if cash_history:
    ...
    plt.title('Cash Over Time (Dual System)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Cash ($)')
  ...
  if equity_history:
    ...
    plt.axhline(y=total_initial_equity, color='r', linestyle='--', label='Initial Equity')
    plt.title('Total Equity Over Time (Dual System)', fontsize=14)
    plt.xlabel('Date')
  ...
  if net_unit_history:
    ...
    plt.title('Position Units Over Time (Dual System)', fontsize=14)
    plt.xlabel('Date')
    plt.ylabel('Units')
    plt.legend()
    plt.grid(True, alpha=0.3)
